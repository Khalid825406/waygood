...
const handleUpload = async () => {
  ...
  const endpoint = title === "Universities" ? "/api/university/upload" : "/api/course/upload";
  ...
  toast({
    ...
    description: (
      <div className="flex items-center gap-2">
        ...
        <span>University data from "{file.name}" uploaded successfully.</span>
      </div>
    ),
  });
};
...
const handleCourseUpload = (file: File) => {
  toast({
    ...
    description: (
      <div className="flex items-center gap-2">
        <CheckCircle className="text-green-500" />
        <span>Course data from "{file.name}" uploaded successfully.</span>
      </div>
    ),
  });
};

// ✅ University Template Download
const handleUniversityTemplateDownload = () => {
  ...
  // 🧩 Demo Sample Row (for testing upload)
  const sampleData = [
    "Harvard University", "HARV001", "https://example.com/harvard.jpg", "Cambridge, USA",
    "Massachusetts Hall, Cambridge, MA 02138", "1636", "Private", "Yes",
    "World-renowned Ivy League university.", 
    "Harvard University is a prestigious institution known for research and innovation.",
    "https://www.harvard.edu", "info@harvard.edu", "+1 617-495-1000", "No",
    "2", "4", "5", "3", "1",
    "Engineering, Business, Law, Medicine",
    "CSE001, BUS001", "50000", "75000", "USD",
    "Fees vary by program.",
    "Minimum GPA 3.0, English proficiency required.",
    "Clubs, Research Centers, Global Community"
  ];

  const csvHeader = headers.map(h => `"${h.replace(/"/g, '""')}"`).join(",");
  const csvRow = sampleData.map(v => `"${v.replace(/"/g, '""')}"`).join(",");
...
const handleCourseTemplateDownload = () => {
  ...
  // 🧩 Demo Sample Row (for upload test)
  const sampleData = [
    "CSE001", "Bachelor of Computer Science", "CS101", "HARV001", "Harvard University",
    "School of Engineering and Applied Sciences", "Computer Science", "Artificial Intelligence",
    "Undergraduate", "Learn AI, programming, and data systems.", "Comprehensive CS program.",
    "Math, Physics", "Problem Solving, Programming", "Lectures, Labs", "Exams, Projects",
    "180", "48", "English", "https://www.harvard.edu/cs101",
    "computer science, ai, machine learning", "Dr. John Doe", "john.doe@harvard.edu",
    "Room 201, Tech Building", "Fall 2025", "2025,2026", "Full-time",
    "55000", "220000", "USD", "100", "USD", "No",
    "Transcripts, SOP, LORs", "90%+ in 12th", "Bachelor’s in Science",
    "7.0", "100", "65", "120", "185", "Cambridge C1",
    "No", "", "No", "", "Yes", "1400", "No", "",
    "Waiver for top scorers", "Yes", "1", "10%", "2025-06-30", "2025-03-31",
    "https://www.harvard.edu/cs101"
  ];
